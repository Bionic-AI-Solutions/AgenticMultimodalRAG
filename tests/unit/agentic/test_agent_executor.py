...
def test_agent_executor_mcp_tool_call():
    ...
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
        mock_post.return_value = MagicMock(json=lambda: {"answer": "It is sunny in Paris."}, status_code=200, raise_for_status=lambda: None)
...
def test_agent_executor_conditional_step():
    ...
    plan = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="tool_call", modality="text", parameters={"tool": "search"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="tool_call", modality="text", parameters={"tool": "search"}, dependencies=[1], trace={}, condition="step_1.result['run'] == True"),
    ], traceability=True)
    executor = AgentExecutor()
...
def test_agent_executor_rerank_step():
    ...
    plan = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="rerank", modality="text", parameters={"input_step": 1}, dependencies=[1], trace={}),
    ], traceability=True)
    executor = AgentExecutor()
    ...
    plan2 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="rerank", modality="text", parameters={"input_step": 99}, dependencies=[], trace={}),
    ], traceability=True)
...
def test_agent_executor_filter_step():
    ...
    plan = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="filter", modality="text", parameters={"input_step": 1, "min_score": 0.8, "metadata": {"label": "important"}}, dependencies=[1], trace={}),
    ], traceability=True)
    executor = AgentExecutor()
    # Mock vector_search to return results
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
    ...
    # Test error: missing input step
    plan2 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="filter", modality="text", parameters={"input_step": 99, "min_score": 0.8}, dependencies=[], trace={}),
    ], traceability=True)
    result2 = executor.execute_plan(plan2, app_id="app1", user_id="user1")
    assert "error" in result2["final_result"]
    ...
    plan3 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="filter", modality="text", parameters={"input_step": 1, "min_score": 0.8}, dependencies=[1], trace={}),
    ], traceability=True)
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
        mock_post.return_value = MagicMock(json=lambda: {"noresults": []}, status_code=200, raise_for_status=lambda: None)
        result3 = executor.execute_plan(plan3, app_id="app1", user_id="user1")
...
def test_agent_executor_aggregate_step():
    ...
    plan = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="graph_query", modality="text", parameters={"related_to": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=3, type="aggregate", modality="text", parameters={"input_steps": [1, 2], "method": "union"}, dependencies=[1, 2], trace={}),
    ], traceability=True)
    executor = AgentExecutor()
    ...
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
        def side_effect(*args, **kwargs):
            if "/query/vector" in args[0]:
                return MagicMock(json=lambda: {"results": [
                    {"doc_id": "a", "score": 0.9},
                    {"doc_id": "b", "score": 0.8}
                ]}, status_code=200, raise_for_status=lambda: None)
            else:
    ...
    plan2 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="graph_query", modality="text", parameters={"related_to": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=3, type="aggregate", modality="text", parameters={"input_steps": [1, 2], "method": "intersection"}, dependencies=[1, 2], trace={}),
    ], traceability=True)
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
        mock_post.side_effect = side_effect
        result2 = executor.execute_plan(plan2, app_id="app1", user_id="user1")
    ...
    plan4 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="aggregate", modality="text", parameters={"input_steps": [1], "method": "bogus"}, dependencies=[1], trace={}),
    ], traceability=True)
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
        mock_post.return_value = MagicMock(json=lambda: {"results": [{"doc_id": "a"}]}, status_code=200, raise_for_status=lambda: None)
    ...
    plan5 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="aggregate", modality="text", parameters={"input_steps": [1]}, dependencies=[1], trace={}),
    ], traceability=True)
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
        mock_post.return_value = MagicMock(json=lambda: {"noresults": []}, status_code=200, raise_for_status=lambda: None)
...
def test_agent_executor_multihop_step():
    ...
    plan = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="graph_query", modality="text", parameters={"related_to": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="multi-hop", modality="text", parameters={"input_step": 1, "hops": 3}, dependencies=[1], trace={}),
    ], traceability=True)
    executor = AgentExecutor()
    ...
    plan2 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="multi-hop", modality="text", parameters={"input_step": 99, "hops": 2}, dependencies=[], trace={}),
    ], traceability=True)
    result2 = executor.execute_plan(plan2, app_id="app1", user_id="user1")
    assert "error" in result2["final_result"]
    ...
    plan3 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="graph_query", modality="text", parameters={"related_to": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="multi-hop", modality="text", parameters={"input_step": 1, "hops": 2}, dependencies=[1], trace={}),
    ], traceability=True)
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
...
def test_agent_executor_llm_call_step():
    ...
    plan = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="vector_search", modality="text", parameters={"query": "foo"}, dependencies=[], trace={}),
        DecompositionStep(step_id=2, type="llm_call", modality="text", parameters={"input_step": 1, "prompt": "Summarize the results."}, dependencies=[1], trace={}),
    ], traceability=True)
    executor = AgentExecutor()
    # Mock vector_search to return results
    with patch("app.agentic.agent_executor.requests.post") as mock_post:
    ...
    # Error: missing input step
    plan2 = DecompositionPlan(plan=[
        DecompositionStep(step_id=1, type="llm_call", modality="text", parameters={"input_step": 99, "prompt": "Summarize the results."}, dependencies=[], trace={}),
    ], traceability=True)
    result2 = executor.execute_plan(plan2, app_id="app1", user_id="user1")
    assert "error" in result2["final_result"] 