...
def test_load_valid_config():
    config = {"default_edge_types": {"a": 1.0, "b": 1.0}, "app_overrides": {"foo": {"b": 2.0}}}
...
def test_parse_weights_and_app_merge():
    config = {"default_edge_types": {"a": 1.0, "b": 0.5}, "app_overrides": {"foo": {"b": 2.0, "c": 3.0}}}
    with tempfile.NamedTemporaryFile("w+", delete=False) as tf:
        write_yaml(tf.name, config)
        loader = EdgeGraphConfigLoader(tf.name)
...
def test_invalid_weights():
    config = {"default_edge_types": {"a": -1.0}, "app_overrides": {}}
    with tempfile.NamedTemporaryFile("w+", delete=False) as tf:
        write_yaml(tf.name, config)
    ...
    os.unlink(tf.name)
    # Test non-float
    config2 = {"default_edge_types": {"a": "bad"}, "app_overrides": {}}
    with tempfile.NamedTemporaryFile("w+", delete=False) as tf:
